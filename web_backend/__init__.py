...
def interceptor():
    ...
    if settings.UserConfig.jwt_on == 1:
        ...
        sql = "select u_id, u_name, u_password, u_salt from user_info where u_name=%s and is_active=1 and is_delete=0"
        ...
        if is_null is None:
            ...
            logger.info("返回信息" + str(data))
            return Response(json.dumps(data), content_type='application/json')
        user_uuid = hashlib.md5((is_null[1] + is_null[2] + str(is_null[0]) + is_null[3]).encode('utf-8')).hexdigest()
